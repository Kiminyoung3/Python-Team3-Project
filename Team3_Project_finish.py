...
def save_to_excel(file_name, index_type, input_value, result_value):
    data = {
        '재해 지수 종류': [index_type],
        '입력값': [input_value],
        '결과값': [result_value]
    ...
    if os.path.exists(file_name):
        # 기존 파일이 있는 경우, 기존 시트에 데이터를 추가합니다.
        with pd.ExcelWriter(file_name, mode='a', engine='openpyxl') as writer:
            df.to_excel(writer, index=False, header=False, sheet_name='Sheet1')
    else:
        # 기존 파일이 없는 경우, 새로운 파일로 저장합니다.
...
def main():
    ...
    while True:
        ...
        if get_choice == "도수율":
            ...
            save_to_excel("results.xlsx", "도수율", f"재해 발생 건수: {number_of_accidents}, 연간 근로시간: {total_work_hours}", frequency_rate)
            print("." * 30)
            print("-" * 10, "계산결과", "-" * 10)
            print("." * 30)
            ...
            save_to_excel("results.xlsx", "강도율", f"근로손실일수: {lost_work_days}, 연간 근로시간: {total_work_hours}", severity_rate)
            print("-" * 10, "계산결과", "-" * 10)
            ...
            save_to_excel("results.xlsx", "연천인율", f"재해 발생 건수: {number_of_accidents}, 연평균 근로자 수: {total_workers}", annual_accident_rate)
            print("-" * 10, "계산결과", "-" * 10)
            ...
            while True:
                ...
                if disaster == "네":
                    ...
                    save_to_excel("results.xlsx", "종합재해지수", f"도수율: {frequency_rate1}, 강도율: {severity_rate1}", comprehensive_disaster_index1)
                    print("-" * 10, "계산결과", "-" * 10)
                    ...
                    save_to_excel("results.xlsx", "종합재해지수", f"도수율: {dosu}, 강도율: {gangdo}", comprehensive_disaster_index2)
                    print("-" * 10, "계산결과", "-" * 10)
